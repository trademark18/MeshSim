...
");
    ...
    public class node
    {
        ...
        public List<int> physNeighbors = new List<int>();
        ...
        public void SendPacket(packet outgoingPacket)
        {
            ...
            List<int> onlinePhysNeighbors = new List<int>(physNeighbors);

            foreach(int offline in RFNET.offlineNodeList)
            {
                onlinePhysNeighbors.Remove(offline);
            }
            ...
            foreach (int physNeighbor in onlinePhysNeighbors)
            {
                ...
                networkOps.meshNet[physNeighbor - 1].ReceivePacket(outgoingPacket);
            }
        ...
        private void ReceivePacket(packet incomingPacket)
        {
            ...
            if (incomingPacket.currentDestination == nodeID || incomingPacket.currentDestination == 255)
            {
                ...
                // Eavesdropping for tracking-enabled nodes
  /*----->*/    if(incomingPacket.finalDestination != nodeID && incomingPacket.packetType == 3 && incomingPacket.payload.Length > 0)
        ...
        public void start()
        {
            ...
            // Stop being offline
            RFNET.offlineNodeList.Remove(nodeID);
    ...
    // Used to serialize the object and store it in XML.
    public class formattedMeshNet
    {
        public List<node> contents;
    }

    // Things related to the network itself
    class networkOps
    {
        //----------  NETWORK SIZE PARAMETERS --------------
        public static int maxNodes = 50;
        public static int minNodes = 50;
        public static int maxNeighbors = 2;
        //--------------------------------------------------
